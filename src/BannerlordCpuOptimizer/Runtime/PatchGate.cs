...
namespace BannerlordCpuOptimizer.Runtime
{
    ...
    {
        internal static bool ValidateProfilerTarget(MethodBase method, ProfilerTargetSpec specification, bool allowUnknownProfilerTargets, out string reason)
        {
            if (method == null) { reason = "target method was not resolved"; return false; }
            if (method.IsAbstract || method.ContainsGenericParameters) { reason = "abstract or open generic methods are not patchable"; return false; }
            if (method.GetMethodBody() == null) { reason = "target has no managed method body"; return false; }
            if (!SignatureMatches(method, specification, out reason)) { return false; }
            ...
            if (!knownModule && !allowUnknownProfilerTargets)
            {
            ...
            if (!string.IsNullOrEmpty(specification.ExpectedIlSha256))
            {
                ...
                {
                    reason = "IL fingerprint mismatch: expected " + specification.ExpectedIlSha256 + ", actual " + (actualHash ?? "<none>");
                    return false;
                }
            }

            reason = knownModule ? "validated known build" : "validated exact signature on explicitly allowed unknown profiler build";
            return true;
        }

        private static bool SignatureMatches(MethodBase method, ProfilerTargetSpec specification, out string reason)
        {
            if (!string.Equals(method.DeclaringType?.FullName, specification.TypeName, StringComparison.Ordinal)) { reason = "declaring type mismatch"; return false; }
            if (!string.Equals(method.Name, specification.MethodName, StringComparison.Ordinal)) { reason = "method name mismatch"; return false; }

            ParameterInfo[] parameters = method.GetParameters();
            if (parameters.Length != specification.ParameterTypeNames.Count) { reason = "parameter count mismatch"; return false; }
        ...
        {
            if (type.IsByRef) { return NormalizeTypeName(type.GetElementType()) + "&"; }
